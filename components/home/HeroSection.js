  ...
  return (
    <header className={`w-full max-w-5xl px-6 py-6 flex justify-end items-center gap-3 absolute top-0 z-50 transition-opacity duration-500 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
      ...
      {!session ? (
        ...
        <Link
          href="/dashboard"
          className="bg-white border border-slate-200 text-blue-600 px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-50 transition-all shadow-sm flex items-center gap-2"
        >
          <User size={14} />
          Meu Painel
        </Link>
