...
export default function HeroSection({ onLog }) {
  ...
  // Filled pill — CTA principal
  const btnPrimary = [
    'flex items-center gap-2 px-4 py-2 rounded-full',
    'bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest',
    'shadow-md shadow-blue-200/60 hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-200/80',
    'transition-all duration-200 active:scale-95',
  ].join(' ')

  // Skeleton pill — mesma proporção dos botões reais
  const skeleton = 'h-8 rounded-full animate-pulse bg-slate-100/70'

  return (
    <header className="w-full max-w-5xl mx-auto px-5 py-5 flex justify-end items-center gap-2 absolute top-0 left-0 right-0 z-50 h-20">

      {/* ── Carregando sessão → skeletons neutros ── */}
      {sessionLoading ? (
        ...
        <>
          {erroLogin && (
            <span className="text-[10px] font-semibold text-red-400 mr-2 animate-in fade-in">
        ...
        /* ── Logado, detectando role → skeleton evita flash do Meu Painel ── */
        <div className="flex gap-2">
          <div className={`${skeleton} w-32`} />
          <div className={`${skeleton} w-28`} />
        </div>
        ...
        <>
          ...
          {/* Explícito: só aparece para prestadores confirmados — nunca para clientes ou null */}
          {role === 'prestador' && (
