...
const FALLBACKS = [
  ...
  },
  {
    ...
    subtitulo: 'Encontre profissionais para este tipo de serviço.',
    cta: 'Ver profissionais',
    // Usa o contexto da busca atual — não carrega tudo, filtra pela categoria
    href: (contexto) => contexto
      ? `/prestadores?q=${encodeURIComponent(contexto)}`
      : '/prestadores',
    cor: 'from-slate-700 to-slate-800',
  },
]

export default function AnuncioCard({ anuncio, contexto = '' }) {
  ...
  useEffect(() => {
    if (!anuncio?.adsense_slot) { setMostrarFallback(true); return }
