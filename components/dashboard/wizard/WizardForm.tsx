...
export function WizardForm({ hookData }: Props) {
  const {
    ...
    prestadorInfo    // ✅ lido de state (não de actions)
  } = hookData.state
  ...
  const {
    ...
    // ✅ prestador removido daqui — não existe em actions
  } = hookData.actions
      ...
      <div className="grid grid-cols-1 md:grid-cols-[240px_1fr] gap-5">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            {/* Logo / foto de perfil */}
            <div className="aspect-square w-full bg-slate-50 border-b border-slate-100 flex items-center justify-center overflow-hidden">
              {prestadorInfo.foto ? (
                <img
            ...
            <div className="px-4 py-3">
              <p className="text-sm font-bold text-slate-900 leading-none">
                {prestadorInfo.nome || 'Seu nome'}
              </p>
              {prestadorInfo.slug && (
                <p className="text-[11px] text-blue-500 mt-0.5">@{prestadorInfo.slug}</p>
              )}
            </div>
          </div>

          {/* WhatsApp do prestador */}
          {prestadorInfo.whatsapp && (
            <div className="bg-white rounded-2xl border border-slate-100 px-4 py-3 flex items-center gap-3">
              <Phone size={14} className="text-slate-400 shrink-0" />
              <div>
                <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400 leading-none mb-1">Whatsapp</p>
                <p className="text-[13px] font-semibold text-slate-800">{prestadorInfo.whatsapp}</p>
              </div>
            </div>
          )}

          {/* Ver meu perfil */}
          <button className="flex items-center justify-center gap-1.5 py-2.5 text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-colors bg-white rounded-2xl border border-slate-100">
        ...
        {/* ════════════════════════════
            ...
            ════════════════════════════ */}
        <div className="flex flex-col gap-5 overflow-y-auto max-h-[82vh] custom-scrollbar pr-1">
