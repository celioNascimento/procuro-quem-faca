...
export default function AvaliacoesDashboardTab({ prestadorId }: { prestadorId: number | null }) {
  ...
  return <section className="flex flex-col gap-6" aria-labelledby="reputacao-titulo">
    ...
    <div className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-2 sm:grid sm:grid-cols-3 sm:overflow-visible sm:pb-0" aria-label="Resumo das avaliações">
      <article className="min-w-[82%] snap-start rounded-3xl border border-slate-200 bg-white p-5 sm:min-w-0"><div className="flex items-center gap-2 text-slate-400"><Star className="size-4 text-amber-500" /><span className="text-xs font-bold uppercase tracking-wide">Nota média</span></div><p className="mt-3 text-4xl font-black text-slate-900">{stats.total ? stats.media.toFixed(1) : '—'}</p><Stars value={stats.media} /></article>
      <article className="min-w-[82%] snap-start rounded-3xl border border-slate-200 bg-white p-5 sm:min-w-0"><div className="flex items-center gap-2 text-slate-400"><MessageSquare className="size-4" /><span className="text-xs font-bold uppercase tracking-wide">Avaliações</span></div><p className="mt-3 text-4xl font-black text-slate-900">{stats.total}</p><p className="mt-1 text-xs text-slate-500">opiniões publicadas</p></article>
      <article className="min-w-[82%] snap-start rounded-3xl border border-slate-200 bg-white p-5 sm:min-w-0"><div className="flex items-center gap-2 text-slate-400"><TrendingUp className="size-4 text-emerald-600" /><span className="text-xs font-bold uppercase tracking-wide">Recomendação</span></div><p className="mt-3 text-4xl font-black text-slate-900">{stats.total ? Math.round((stats.totalIndica / stats.total) * 100) : 0}%</p><p className="mt-1 text-xs text-slate-500">dos clientes indicam</p></article>
    </div>
