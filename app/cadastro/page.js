...
function SenhaInput({ value, onChange, placeholder, className, required, erro }) {
...
// ─── Versão para fundo azul (seção "Crie seu Acesso") ────────────────────────
function SenhaInputAzul({ value, onChange, placeholder, className, required }) {
  const [visivel, setVisivel] = useState(false)
  return (
    <div className="relative">
      <input
...
function FormularioCadastro() {
  ...
  const carregarCidades = useCallback(async (regiaoId, estadoSigla) => {
    ...
    if (regiaoId) {
      ...
      setListaCidades([]); setCidadesRegiao([]); return;
    }
      ...
      try {
        ...
        if (perfilParaCarregar) {
          ...
          setFormData({
            ...
            whatsapp: aplicarMascaraWhatsapp(perfilParaCarregar.whatsapp) || '',
          ...
          setFormData(prev => ({ ...prev, nome: nomeSocial, slug: formatarParaSlug(nomeSocial) }));
  ...
  const handleEstadoChange = async (e) => {
    ...
    setFormData(prev => ({ ...prev, estado_sigla: novaSigla, regiao_id: '', cidade_id: '', bairro: '', cidades_atendidas: [] }));
  ...
  const handleRegiaoChange = async (e) => {
    ...
    setFormData(prev => ({ ...prev, regiao_id: novoRegiaoId, cidade_id: '', bairro: '', cidades_atendidas: [] }));
    await carregarCidades(novoRegiaoId, formData.estado_sigla);
  };

  const handleGrupoChange = async (e) => {
    const novoGrupoId = e.target.value;
    setFormData(prev => ({ ...prev, grupo_id: novoGrupoId, categoria_id: '', habilidades: [] }));
    await carregarCategorias(novoGrupoId);
  };

  const toggleItem = (item, lista) => {
    const listaAtual = formData[lista] || [];
    const novaLista = listaAtual.includes(item) ? listaAtual.filter(i => i !== item) : [...listaAtual, item];
  ...
  const fazerUploadFoto = async (e) => {
    try {
      ...
      setErrorModal({ show: true, title: 'Erro no Upload', message: 'Não conseguimos processar sua imagem. Tente novamente ou use outro arquivo.', actionText: 'Entendido' })
  ...
  const calcularProgresso = () => {
    // Para usuário novo: senha E confirmação devem bater
    const senhaNovoOk = !userLogado
      ? (email.includes('@') && senha.length >= 6 && senha === confirmarSenha)
      : true;
    const campos = [
  ...
  async function handleSubmit(e) {
    ...
    if (!userLogado) {
      if (senha.length < 6) { setStatus('❌ A senha deve ter no mínimo 6 caracteres.'); return; }
      if (senha !== confirmarSenha) { setStatus('❌ As senhas não coincidem.'); return; }
    }

    if (userLogado && (senha.length > 0 || confirmarSenha.length > 0)) {
      if (senha.length < 6) { setStatus('❌ A nova senha deve ter no mínimo 6 caracteres.'); return; }
      if (senha !== confirmarSenha) { setStatus('❌ As senhas não coincidem.'); return; }
    }
  ...
  // ── Classe base dos inputs de senha na seção azul (fundo branco, sem borda) ──
  const senhaInputAzulClass = `w-full p-4 rounded-2xl border-none outline-none font-medium text-[14px] text-slate-800 bg-white shadow-inner`
        ...
        >
          <form onSubmit={handleSubmit} className="grid grid-cols-12 gap-8">
            ...
            <div className="col-span-12 md:col-span-8 space-y-6">
              ...
              {/* Seção de acesso — usuário NÃO logado */}
              {!userLogado && (
                <section className="bg-blue-600 p-8 rounded-[2.5rem] shadow-xl shadow-blue-100 space-y-4">
                  ...
                  <input
                    ...
                    className="w-full p-4 rounded-2xl border-none outline-none font-medium text-[14px] text-slate-800 bg-white shadow-inner"
                    required
                  />
                  {/* ── NOVO: grade 2 colunas com confirmação ── */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <SenhaInputAzul
                      value={senha}
                      onChange={e => setSenha(e.target.value)}
                      placeholder="Senha (mín. 6 caracteres)"
                      className={`${senhaInputAzulClass} ${tentouEnviar && senha.length < 6 ? 'ring-2 ring-red-400' : ''}`}
                      required
                    />
                    <SenhaInputAzul
                      ...
                      className={`${senhaInputAzulClass} ${tentouEnviar && senha !== confirmarSenha ? 'ring-2 ring-red-400' : ''}`}
                      required
                    />
                  </div>
                  {/* Feedback inline de senhas divergentes */}
              ...
              {/* Seção de acesso — usuário JÁ logado */}
              {userLogado && (
                ...
                <div className="space-y-4">
                  <input
                    ...
                    onChange={e => {
                      ...
                      setFormData(prev => ({ ...prev, nome: novoNome, slug: editouSlugManualmente ? prev.slug : formatarParaSlug(novoNome) }));
                    }}
                    className={inputStyle()}
                    required
                  />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <select
                    ...
                    onChange={e => {
                      ...
                      setFormData({ ...formData, cidade_id: cidId, cidades_atendidas: formData.cidades_atendidas.filter(n => n !== nomeSede) });
                    }}
                    className={inputStyle()}
                    required
              ...
              <div className="flex flex-col items-center">
                {/* ── CORRIGIDO: cor do status muda conforme erro/sucesso ── */}
                {status && (
                  <div className={`w-full mb-4 p-4 rounded-2xl text-[10px] font-black text-center uppercase tracking-wider animate-in fade-in ${
                    status.startsWith('❌')
