...
export default function PaginaAvaliacaoCliente({ params: paramsPromise }) {
  ...
  const handleFinalizarAvaliacao = async () => {
    ...
    setSubmitting(true)
    try {
      // 1. Registro da Avaliação com definição explícita de status
      // Note que adicionamos 'status: finalizado' para bater com o fluxo do app
      const { error: errorAval } = await supabase.from('avaliacoes').insert({
        projeto_id: projeto.id,
        prestador_id: projeto.prestador_id,
        nota: nota,
        comentario: comentarioGeral,
        visivel: true,
        status: 'finalizado' // <--- ADICIONADO: Garante que o status na tabela 'avaliacoes' mude
      })

      if (errorAval) throw errorAval

      // 2. Atualização de status na tabela 'portfolio_projetos'
      const { error: updateError } = await supabase.from('portfolio_projetos')
        .update({
          status: 'finalizado',
          data_conclusao: new Date().toISOString().split('T')[0]
        })
        .eq('id', projeto.id)
        .eq('avaliacao_token', token) // Reforço de segurança

      if (updateError) throw updateError

      // Sincronização do estado local
      ...
      // Opcional: Adicione um feedback visual de erro aqui para o usuário
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="min-h-screen bg-[#FDFDFD] text-slate-900 pb-20 font-sans antialiased">
          ...
          <div className="bg-blue-600 p-8 rounded-[2.5rem] text-white shadow-xl shadow-blue-100">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest">Sua Avaliação Final</span>
              </div>
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map(s => <Star key={s} size={12} fill={avaliacaoExistente.nota >= s ? "white" : "transparent"} stroke="white" strokeWidth={2.5} />)}
              </div>
            </div>
            <p className="text-[15px] font-medium leading-relaxed opacity-90 italic">"{avaliacaoExistente.comentario || "Serviço finalizado com excelência."}"</p>
          </div>
        )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white p-5 rounded-[2.2rem] border border-slate-100 flex items-center gap-4 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-[13px] font-bold text-slate-800 uppercase leading-none">{labelEtapaAtual}</p>
                </div>
              </div>
              <div className="bg-white p-5 rounded-[2.2rem] border border-slate-100 flex items-center gap-4 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-[13px] font-bold text-slate-800 uppercase leading-none">{fotosOrdenadas.length} de 3</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h2 className="text-base font-bold text-slate-800 uppercase leading-none truncate">{projeto.prestadores?.nome}</h2>
                  <p className="text-[13px] font-medium text-slate-500 mt-2 truncate">{projeto.titulo}</p>
                </div>
                <a href={`tel:${projeto.prestadores?.whatsapp}`} className="w-12 h-12 bg-green-500 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-green-100 active:scale-95 transition-all">
                ...
                <button key={foto.id} onClick={() => setFotoSelecionada(foto)} className="aspect-square rounded-[2rem] overflow-hidden bg-white border-2 border-white shadow-md relative group active:scale-95 transition-all">
                  ...
                  {comentarios.filter(c => c.foto_id === foto.id).length > 0 && (
                    <div className="absolute top-2 right-2 w-6 h-6 bg-blue-600 rounded-full flex items-center justify-center border-2 border-white shadow-lg">
                      <span className="text-[10px] font-bold text-white">{comentarios.filter(c => c.foto_id === foto.id).length}</span>
                    </div>
                  )}
                </button>
              ))}
          ...
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
            <div className="bg-white border border-slate-100 rounded-[3rem] overflow-hidden shadow-2xl">
              <div className="p-6 border-b border-slate-50 flex items-center gap-3">
                <div className="w-9 h-9 rounded-full p-[2px] bg-blue-600">
                  <img src={projeto.prestadores?.foto_perfil} className="w-full h-full rounded-full object-cover border-2 border-white" />
                </div>
                <p className="text-[13px] font-bold text-slate-800 uppercase">{projeto.prestadores?.nome}</p>
              </div>
              ...
              <div className="relative aspect-square bg-slate-50 flex items-center justify-center">
                ...
                <div className="absolute top-6 right-6 bg-black/60 backdrop-blur-md text-white px-4 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-widest border border-white/20">
              ...
              <div className="p-8 text-center bg-white">
                <h4 className="text-[14px] font-bold text-slate-800 uppercase tracking-tight">{projeto.titulo}</h4>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">Relatório de Entrega Finalizado</p>
              </div>
            </div>
          </div>
        )}
        ...
        {!visualmenteConcluido && temConclusao && (
          <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-2xl shadow-blue-100/20 space-y-8">
            <div className="text-center">
              <h3 className="text-2xl font-bold text-slate-800 tracking-tight">Assinar Entrega</h3>
            ...
            <div className="flex justify-center gap-3">
              {[1, 2, 3, 4, 5].map(star => (
                <button key={star} onClick={() => setNota(star)} onMouseEnter={() => setHoverNota(star)} onMouseLeave={() => setHoverNota(0)} className="transition-transform active:scale-90">
                  <Star size={40} fill={(hoverNota || nota) >= star ? "#2563eb" : "transparent"} color={(hoverNota || nota) >= star ? "#2563eb" : "#E2E8F0"} strokeWidth={2} />
                </button>
              ))}
            </div>

            <textarea
              className="w-full p-6 bg-slate-50 rounded-[2.2rem] border border-slate-100 outline-none text-[14px] font-medium text-slate-700 min-h-[140px] resize-none focus:border-blue-200 transition-all placeholder:text-slate-300 shadow-inner"
            ...
            <button
              ...
              className="w-full py-6 bg-blue-600 text-white rounded-[2rem] font-bold uppercase text-[11px] tracking-[0.2em] flex items-center justify-center gap-3 disabled:opacity-40 shadow-xl shadow-blue-100 active:scale-[0.98] transition-all"
      ...
      {fotoSelecionada && !visualmenteConcluido && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/95 backdrop-blur-md p-4 animate-in fade-in">
          <div className="w-full max-w-5xl bg-white rounded-[3rem] overflow-hidden flex flex-col md:flex-row h-[85vh] shadow-2xl">
            ...
            <div className="w-full md:w-[400px] bg-white flex flex-col">
              <div className="p-6 border-b border-slate-50 flex items-center justify-between">
                <h3 className="text-[11px] font-bold uppercase tracking-widest text-slate-800">Discussão Técnica</h3>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 bg-slate-50 p-4 rounded-2xl rounded-tl-none border border-slate-50 shadow-sm">
                ...
                {comentarios.filter(c => c.foto_id === fotoSelecionada.id).map(com => (
                  <div key={com.id} className={`flex gap-3 ${com.autor_tipo === 'cliente' ? 'flex-row-reverse' : ''}`}>
                    <div className={`flex-1 p-4 rounded-2xl border ${com.autor_tipo === 'cliente' ? 'bg-blue-600 text-white border-blue-500 rounded-tr-none shadow-md' : 'bg-slate-50 text-slate-700 border-slate-100 rounded-tl-none shadow-sm'}`}>
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-50">
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-blue-600 text-white rounded-xl active:scale-90 transition-all disabled:opacity-30 shadow-md"
                  >
