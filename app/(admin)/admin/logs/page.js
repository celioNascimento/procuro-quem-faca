...
export default function AdminLogs() {
  ...
  const exportarCSV = () => {
    ...
    link.setAttribute("download", `logs_auditoria_${new Date().getTime()}.csv`)
    link.click()
  }
      ...
      <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-sm mb-6">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl">
            {['DIA', 'MES'].map(p => (
              <button key={p} onClick={() => setPeriodoGrafico(p)} className={`px-4 py-1.5 rounded-lg text-[9px] font-black transition-all ${periodoGrafico === p ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{p}</button>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-100 rounded-[2.5rem] shadow-sm overflow-hidden mb-20">
        <div className="p-6 border-b border-slate-50 flex flex-col md:flex-row gap-4 justify-between items-center bg-slate-50/30">
          ...
          {(busca || dataInicio || tipoFiltro !== 'TODOS') && (
            <button onClick={() => { setBusca(''); setDataInicio(''); setTipoFiltro('TODOS'); }} className="text-[9px] font-black text-indigo-600 uppercase border-b-2 border-indigo-600">Resetar Filtros</button>
          )}
        </div>
