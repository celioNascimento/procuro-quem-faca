...
export default function ModeracaoPrestadores() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10 bg-[#F4F7F9] min-h-screen font-sans text-slate-800">
        ...
        <div className="flex flex-col gap-3 mt-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <select 
              ...
              className="p-4 rounded-2xl border border-slate-200 font-bold text-xs bg-white text-slate-800 outline-none shadow-sm appearance-none"
            >
            ...
            <input 
              ...
              className="p-4 rounded-2xl border border-slate-200 font-bold text-xs bg-white text-slate-800 outline-none shadow-sm"
            />
          </div>
        </div>
      </header>
      ...
      <div className="grid gap-4 sm:gap-6">
        {loading && prestadores.length === 0 ? (
          <div className="py-20 text-center animate-pulse font-black text-slate-400 uppercase tracking-widest text-xs">Sincronizando dados...</div>
        ) : listaExibida.map(p => {
              ...
              <div className="flex flex-col lg:flex-row items-center lg:items-center gap-5">
                ...
                <div className="flex lg:block items-center justify-between w-full lg:w-auto">
                  ...
                  <div className="lg:hidden flex flex-col items-end gap-2">
                    <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest ${statusEstilo.bg} ${statusEstilo.text}`}>
                      {statusEstilo.label}
                    </span>
                    {temDenunciaAberta && (
                      <div className="bg-red-600 text-white px-2 py-1 rounded-lg text-[8px] font-black animate-pulse">
                ...
                <div className="flex-1 w-full">
                  <div className="hidden lg:flex items-center gap-3 mb-2">
                    <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight italic">{p.nome || 'SEM NOME'}</h3>
                    <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest ${statusEstilo.bg} ${statusEstilo.text}`}>
                      {statusEstilo.label}
                    </span>
                  </div>
                  ...
                  <div className="lg:hidden mb-4">
                     <h3 className="text-lg font-black text-slate-900 uppercase italic leading-none">{p.nome || 'SEM NOME'}</h3>
                  </div>

                  {editando === p.id ? (
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 bg-slate-50 p-4 rounded-2xl border border-blue-100">
                      <select id={`cat-${p.id}`} defaultValue={p.categoria} className="sm:col-span-2 p-3 rounded-xl text-[10px] font-bold text-slate-800 border bg-white">{CATEGORIAS_OFICIAIS.map(c => <option key={c} value={c}>{c}</option>)}</select>
                      <input id={`b-${p.id}`} defaultValue={p.bairro} placeholder="Bairro" className="p-3 rounded-xl text-[10px] font-bold text-slate-800 border bg-white" />
                      <select id={`c-${p.id}`} defaultValue={p.cidade_id} className="p-3 rounded-xl text-[10px] font-bold text-slate-800 border bg-white">
                        <option value="">Cidade</option>
                        {cidades.map(cid => <option key={cid.id} value={cid.id}>{cid.nome}</option>)}
                      </select>
                      <div className="sm:col-span-2 flex gap-2 pt-2">
                        <button onClick={() => setEditando(null)} className="flex-1 bg-slate-200 text-slate-600 p-3 rounded-xl text-[10px] font-black uppercase">Cancelar</button>
                        <button onClick={() => salvarEdicao(p.id)} className="flex-1 bg-blue-600 text-white p-3 rounded-xl text-[10px] font-black uppercase">Salvar</button>
                      </div>
                    </div>
                  ) : (
                    <div className="flex flex-wrap gap-2 sm:gap-4 text-[9px] sm:text-[10px] font-bold uppercase text-slate-400">
                  ...
                  <div className="flex gap-2 items-center">
                    {temDenunciaAberta && (
                      <button 
                        ...
                        className="hidden lg:block bg-red-600 text-white px-4 py-4 rounded-2xl font-black text-[10px] uppercase shadow-lg shadow-red-100 hover:scale-105 transition-transform"
                      >
                        🚨 Ver Denúncias
                      </button>
                    )}
                    <button 
                      ...
                      className={`px-6 py-4 rounded-2xl font-black text-[10px] uppercase shadow-xl transition-all active:scale-95 ${p.status === 'ativo' ? 'bg-slate-200 text-slate-500' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-blue-100'}`}
                    >
                ...
                {/* Botão Mobile de Denúncias */}
                {temDenunciaAberta && (
                  <button 
                    onClick={() => setDenunciasSelecionadas({ lista: p.listaDenuncias, id: p.id })}
                    className="lg:hidden w-full py-3 bg-red-50 text-red-600 rounded-xl font-black text-[9px] uppercase tracking-widest border border-red-100"
                  >
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setDenunciasSelecionadas(null)}>
          <div className="bg-white w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom sm:zoom-in-95" onClick={e => e.stopPropagation()}>
            <div className="p-6 sm:p-8 border-b border-slate-50 flex justify-between items-center">
              <div>
                <h2 className="font-black text-slate-900 uppercase italic tracking-tighter">Central de Denúncias</h2>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Análise de Segurança</p>
              </div>
            ...
            <div className="p-6 sm:p-8 max-h-[70vh] overflow-y-auto space-y-4 scrollbar-hide">
              {denunciasSelecionadas.lista.map((d, i) => (
                <div key={i} className="bg-slate-50 p-5 rounded-[2rem] border border-slate-100 relative overflow-hidden group">
                  ...
                  <p className="text-slate-700 text-xs font-bold italic leading-relaxed">"{d.motivo}"</p>
                  <p className="text-[9px] text-slate-400 font-black uppercase mt-3 flex justify-between">
              ...
              <div className="pt-4 space-y-3">
                <button 
                  ...
                  className="w-full py-5 bg-green-500 text-white rounded-2xl font-black text-[10px] uppercase hover:bg-green-600 transition-all shadow-lg shadow-green-100 active:scale-95"
                >
                ...
                <button 
                  ...
                  className="w-full py-4 text-red-500 font-black text-[10px] uppercase tracking-widest hover:bg-red-50 rounded-2xl transition-colors"
                >
