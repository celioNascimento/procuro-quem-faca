...
export default function Home() {
  ...
  // 2. FUNÇÃO DA BARRA DE BUSCA CORRIGIDA PARA REGIÃO E ESTADO
  const dispararBusca = async (e) => {
    ...
    // Busca exata ou aproximada para identificar a intenção
    ...
    // Adiciona filtros baseados nos resultados encontrados
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] flex flex-col items-center font-sans overflow-x-hidden relative">
      ...
      <section className="w-full max-w-[90%] md:max-w-2xl px-4 pt-24 md:pt-40 pb-10 flex flex-col items-center text-center">
        ...
        {/* LOGO COM DIV ARREDONDADA */}
        <div className="mb-12 md:mb-16 transition-all hover:scale-[1.05]">
          <Link href="/" className="block p-4 bg-white rounded-[2.5rem] shadow-xl shadow-blue-900/5 border border-slate-50 overflow-hidden">
            <img src="/logo.png" alt="Logo" className="h-20 md:h-28 w-auto object-contain" />
          </Link>
        </div>

        {/* BARRA DE BUSCA AJUSTADA NO MOBILE */}
        <form onSubmit={dispararBusca} className="w-full relative group mb-10">
          <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 to-cyan-500 rounded-[1.5rem] md:rounded-[2rem] blur opacity-10 group-hover:opacity-25 transition duration-1000 group-focus-within:opacity-30"></div>
          
          <div className="relative">
            <input
              type="text"
              placeholder="Busque por serviço, cidade..."
              value={busca}
              onChange={(e) => setBusca(e.target.value)}
              // Reduzido h-16 para h-14 no mobile e ajuste de padding pr-24
              className="w-full h-14 md:h-20 pl-6 pr-24 md:pl-8 md:pr-40 rounded-[1.2rem] md:rounded-[2.2rem] border border-slate-200 bg-white shadow-2xl shadow-blue-900/5 outline-none focus:border-blue-500 text-sm md:text-xl transition-all placeholder:text-slate-300"
            />
            <button 
              type="submit"
              // Ajustado posicionamento (right-1.5) e arredondamento (rounded-xl) no mobile
              className="absolute right-1.5 top-1.5 bottom-1.5 bg-blue-600 text-white px-4 md:px-10 rounded-xl md:rounded-[1.7rem] font-black text-[10px] md:text-xs uppercase tracking-[0.15em] active:scale-95 transition-all shadow-lg shadow-blue-200 hover:bg-blue-700"
            >
              BUSCAR
            </button>
          </div>
